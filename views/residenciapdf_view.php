...
$vocero=$firmante->consultarFirmanteCuentadantesVocero();
$restriccion=$constancia->verificarRestriccionConstancia(date('m'),'Residencia',$_REQUEST['dni']);
if($restriccion==0)
	$nuevaConstancia=$constancia->agregarConstancia('Residencia',$ciudadano[0]['dni']);
Historial::Movimiento($_SESSION['codigo'],"Generó constancia de residencia al ciudadano del Documento de Identidad: ".$ciudadano[0]['dni']);
if(date('m')==1)
	$mes='Enero';
else if(date('m')==2)
	$mes='Febrero';
...
<div id='titulo'>CONSTANCIA DE RESIDENCIA</div>
<div>
<div id='cuerpo'>
<?php echo "Quien suscribe, Consejo Comunal ".$config[0]['nombre_consejo_comunal'].", ubicado en ".$config[0]['direccion'].", Parroquia ".$parroquia[0]['nombre'].", Municipio ".$municipio[0]['nombre'].", ".$config[0]['ciudad']." Estado ".$config[0]['estado'].",
 debidamente registrado ante el Sistema de Taquilla Única de Registro, el día ".$fecha_taq[0]." de $mes_taq de ".$fecha_taq[2]." bajo el N° ".$config[0]['nro_reg_taq_unica']; ?> 
...
<?php echo "está actualmente domiciliado en la comunidad de:<b> ".$ciudadano[0]['direccion'].", Sector \"".$ciudadano[0]['sector']."\",";
...
$anio_resid=date('Y')-$ciudadano[0]['anio_residencia'];
if($anio_resid==0 ||$anio_resid==1||empty($ciudadano[0]['anio_residencia']))
	echo " 1 año.";
else
	if(($anio_resid)<$ciudadano[0]['edad'])
		echo $anio_resid." años.";
	else
		echo ($anio_resid-1)." años.";
?></b>
</div>
<div id='cuerpo2'>
