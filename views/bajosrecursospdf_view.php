...
include('../models/municipio_model.php');
ob_start();
...
$municipio=$objMunicipio->consultarMunicipioPorId($config[0]['municipio']);
$parroquia=$objParroquia->consultarParroquiaPorId($config[0]['parroquia']);
$ciudadano=$persona->consultarPersonaDni($_REQUEST['dni']);
$vocero=$firmante->consultarOtroFirmanteVocero();
$restriccion=$constancia->verificarRestriccionConstancia(date('m'),'Bajos recursos',$_REQUEST['dni']);
if($restriccion==0)
	$nuevaConstancia=$constancia->agregarConstancia('Bajos recursos',$ciudadano[0]['dni']);
Historial::Movimiento($_SESSION['codigo'],"Generó constancia de bajos recursos al ciudadano del Documento de Identidad: ".$ciudadano[0]['dni']);
if(date('m')==1)
	$mes='Enero';
else if(date('m')==2)
	$mes='Febrero';
...
<div id='titulo'>CONSTANCIA DE BAJOS RECURSOS</div>
<div>
<div id='cuerpo'>
<?php ?>Quien suscribe, Consejo Comunal "<?php echo $config[0]['nombre_consejo_comunal']."\", ubicado en ".$config[0]['direccion'].", Parroquia ".$parroquia[0]['nombre'].", Municipio ".$municipio[0]['nombre'].", ".$config[0]['ciudad']." Estado ".$config[0]['estado']; ?>
, con Registro de Información Fiscal - RIF N°: <b><?php echo $config[0]['rif']; ?>,</b> hace constar que
<?php 
...
<?php echo " docimiciliado en la comunidad de:<b> ".$ciudadano[0]['direccion'].", Sector \"".$ciudadano[0]['sector']."\",";
if(empty($ciudadano[0]['numero_casa']))
	echo " casa S/N";
else
	echo " casa N° ".$ciudadano[0]['numero_casa'];
echo ", Parroquia ".$ciudadano[0]['nombre_parroquia'].", Municipio ".$ciudadano[0]['nombre_municipio'].", Carúpano Estado ".$ciudadano[0]['nombre_estado']."</b>, es una persona de bajos recursos y no posee ningún tipo de ingreso económico.";?>
</div>
<div id='cuerpo2'>
Constancia que se expide de parte interesada en la ciudad de <?php echo $config[0]['ciudad']; ?> Estado <?php echo $config[0]['estado']; ?> el <?php echo date('d')." de ".$mes." de ".date('Y')."."; ?> 
</div>
</div>
...
<hr class='firma'>	<?php 
	if(count($vocero)==3)
	{
	echo $vocero[0]['nombre']." ".$vocero[0]['apellido']."<br>".$vocero[0]['tipo_dni']."-".$vocero[0]['dni_vocero']."<br>".$vocero[0]['telefono_vocero']."<BR>".ucwords(strtolower($vocero[0]['nombre_voceria'])); 
	?>
	...
	<div id='firmante2'>
<hr class='firma'>
	<?php echo $vocero[1]['nombre']." ".$vocero[1]['apellido']."<br>".$vocero[1]['tipo_dni']."-".$vocero[1]['dni_vocero']."<br>".$vocero[1]['telefono_vocero']."<BR>".ucwords(strtolower($vocero[1]['nombre_voceria']));  ?>
	</div>
	<div id='firmante3'>
<hr class='firma'>	
	<?php echo $vocero[2]['nombre']." ".$vocero[2]['apellido']."<br>".$vocero[2]['tipo_dni']."-".$vocero[2]['dni_vocero']."<br>".$vocero[2]['telefono_vocero']."<BR>".ucwords(strtolower($vocero[2]['nombre_voceria'])); 
